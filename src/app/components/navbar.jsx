        ...
        <div className="h-full flex items-center justify-between px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48">
            ...
            <div className="md:hidden z-10">
                <button className="flex flex-col justify-between w-10 h-8 z-50 relative" onClick={() => setOpen((prev) => !prev)}>
                    <motion.div animate={open ? "opened":"closed"} variants={topVariants} className="bg-black rounded h-1 w-10 origin-left"></motion.div>
                    <motion.div animate={open ? "opened":"closed"} variants={centerVariants} className="bg-black rounded h-1 w-10"></motion.div>
                    <motion.div animate={open ? "opened":"closed"} variants={bottomVariants} className="bg-black rounded h-1 w-10 origin-left"></motion.div>
                </button>
                {/* Menu List */}
                {open && ( 
                <motion.div variants={listVariants} animate="opened" initial="closed" className="absolute top-0 left-0 w-screen h-screen bg-black text-white flex flex-col items-center justify-center gap-8 text-4xl">
                    {links.map(linkItem => (<motion.div variants={listItemVariants} key={linkItem.url}><Link href={linkItem.url}>{linkItem.title}</Link></motion.div>))}
                </motion.div>)}
            </div>
